...
Use `gemini_informal_prover` or `discussion_partner` to get guidance.
...
| Use `gemini_informal_prover` for proofs | Use wrong tool | Right tool gives better results |
| Use `discussion_partner` for brainstorming | Skip exploration | Open discussion finds creative approaches |
